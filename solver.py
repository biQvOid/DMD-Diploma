...
def function_field(x, y, t):
    if x == 17 and y == 17:
        ...
        return 250000 * np.cos(80 * t + np.pi / 4)
    elif x == 27 and y == 27:
        return 1000000 * np.cos(150 * t + np.pi / 2)
    elif x == 30 and y == 30:
        return 200000 * np.cos(40 * t + np.pi / 6)
...
def function_field(x, y, t):
    if x == 17 and y == 17:
        return 150000 * np.cos(20 * t + np.pi / 3)
    elif x == 50 and y == 50:
        return 300000 * np.cos(20 * t + np.pi / 8)
    elif x == 33 and y == 33:
        return 250000 * np.cos(20 * t + np.pi / 4)
    elif x == 27 and y == 27:
        return 1000000 * np.cos(20 * t + np.pi / 2)
    elif x == 30 and y == 30:
...
source_info = [(7, 7), (12, 12)]

modes = np.array([np.reshape(u[i, :, :], (3600,)) for i in range(u.shape[0])])
matrix = modes.T
#dmd(matrix[:, :-1], matrix[:, 1:], truncate=3)
snaphots_research(u[30:, :, :], 4, "5_sources(27, 27)", hx, hy, tau, source_info, save=False, save_modes_=False, save_snapshots_=False, save_evolution_=True)